...
func matchQueryParams(values url.Values, requestStats *analysis.RequestStats) {
	for k := range values {
		if hits, ok := requestStats.Query[k]; ok {
			...
			glog.Errorf("Invalid query param: '%s' for '%s %s'", k, requestStats.Method, requestStats.Path)
		}
	}
}
...
func matchBodyParams(requestObject *runtime.Unknown, requestStats *analysis.RequestStats) error {
	if requestObject != nil && requestStats.Body != nil {
		...
		glog.Warningf("Request '%s %s' should not contain body params", requestStats.Method, requestStats.Path)
	}
...
func calculateCoverage(restAPIStats map[string]map[string]*analysis.RequestStats) map[string]float64 {
